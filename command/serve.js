...
module.exports = {
  ...
  exec: async command => {
    ...
    let instances = command.args.action ? coreConfig.serverInstances : 1
    let ecosystem = {
      apps: [{
        ...
        exec_mode: 'cluster',
        instances,
        watch: [`${process.cwd()}/core/${smol.coreName}`, `${process.cwd()}/config`],
        watch_delay: 1000,
      }],
    }
    ...
    // normal serve
    if (!command.args.action) command.run(`pm2 start ${ecosystemPath} --no-daemon`)
